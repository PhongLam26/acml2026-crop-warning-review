...
- `outputs/reports/`: minimal baseline-suite configuration and split identifiers needed by the reviewer checks.

Raw source tables and intermediate report logs are intentionally excluded from this review snapshot; the included processed panels are sufficient to reproduce the model checks reported below.
